...
def infer_input_model(
    ...
    *,
    ...
    fn_name = getattr(fn, "__name__", "tool")
    model = create_model(f"{fn_name}_parameters", __config__=ConfigDict(extra="forbid"), **fields)
    return model, takes_state
...
def infer_tool_schema(
    ...
    *,
    ...
    """Return the same schema used to hydrate runtime arguments."""
    model, takes_state = infer_input_model(fn, doc_params=doc_params)
