...
*   **Tool Concurrency Limit**: Set `MAX_PARALLEL_TOOLS` (default: `4`) to bound the number of tool calls executed concurrently within one loop iteration.
...
*   **Tool Timeout**: Set `TOOL_TIMEOUT` (default: `30.0` seconds) to prevent a single slow tool from holding a loop indefinitely. Set to `0` to disable. A timeout cannot forcibly terminate a synchronous Python thread that is already blocked in external I/O, so long-running network, database, or subprocess tools should be async or use their own client-level timeouts.
*   **Tool Payload Limits**: Set `MAX_TOOL_ARGS_CHARS` (default: `20000`) and `MAX_TOOL_OUTPUT_CHARS` (default: `8192`) to protect message history from tool-call payload blowups.
*   **Context Token Budget**: The loop prunes oldest non-prefix message groups when the estimated history exceeds `max_context_tokens` (default: `8192` tokens, set on `ToolExecutionConfig`). This stops runaway loops before they exceed a model context window or create unexpected token cost; the system prompt, context items, and user objective are never pruned.
